...
export async function POST(req: NextRequest) {
  try {
    const { orderId, methodTitle, total, courierName, serviceName } = await req.json();
    ...
    const methodTitleStr = courierName && serviceName ? `${courierName} — ${serviceName}` : methodTitle || "";
    const totalStr = String(Math.round(parseFloat(total) || 0));
    ...
    console.error("Update order shipping error:", e);
