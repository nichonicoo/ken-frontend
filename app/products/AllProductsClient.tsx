...
export default function AllProductsClient({ products, search }: Props) {
  ...
  // Reset price range when products change (e.g., new search results)
  useEffect(() => {
    setPriceReady(false);
    setPriceRange([0, 0]);
  }, [search]);
      ...
      <div style={styles.breadcrumbWrap}>
        <Breadcrumb
          items={[
            ...
            search ? { label: `Hasil Pencarian: '${search}'` } : { label: "All Products" },
          ]}
        />
      </div>

      {/* Title */}
      <h1 style={styles.title} className={cormorant.className}>
